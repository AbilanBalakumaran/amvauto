"""Catalogue d'animés pré-embarqué pour la démo web.

La démo publiée ne peut pas appeler l'API en direct (les pages Artifact sont
isolées du réseau) : on fige donc une sélection de séries au moment du build.
Le CLI, lui, interroge Sakugabooru en direct et n'est pas limité à cette liste.

Chaque entrée : (nom affiché, requête envoyée à Sakugabooru, alias de recherche).
"""
...
def find_curated(query: str) -> tuple[str, str] | None:
    """(nom affiché, requête sakuga) pour un animé du catalogue, sinon None.

    Sert de garde-fou au CLI : quelques séries portent sur Sakugabooru un tag
    masqué ou volontairement déformé (homoglyphes, titre de code) qu'aucune
    recherche par nom ne retrouve. Le catalogue les rattrape.
    """
    needle = query.strip().lower()
    for display, tag, aliases in SERIES:
        haystack = [display.lower(), *(a.lower() for a in aliases)]
        if any(needle == item for item in haystack):
            return display, tag
    for display, tag, aliases in SERIES:
        haystack = [display.lower(), *(a.lower() for a in aliases)]
        if any(needle in item or item in needle for item in haystack):
            return display, tag
    return None
