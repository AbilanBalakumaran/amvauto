...
import { pousser } from "./pousser.js";
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname.startsWith("/api/")) {
      ...
      try {
        ...
        if (url.pathname === "/api/grenier") return await grenier(request, url, env, ctx);
        if (url.pathname === "/api/pousser") return await pousser(request, url, env);
