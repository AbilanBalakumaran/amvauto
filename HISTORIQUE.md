# Historique — ce qui a été essayé, mesuré et abandonné

Ce fichier est le journal de bord d'`amvauto`. Il s'écrit par le bas, et il garde
la trace de tout ce qui a été construit, mesuré, puis gardé ou retiré : l'ancien
explorateur de rushs, l'atelier de montage à la main — piste multipiste, pellicule
d'imagettes, poignées de rognage, ciseaux, zoom au pincement —, les cinq rounds de
pixellisation, les mesures de lecture sur iPhone, les pistes écartées.

**Rien de ce qui suit ne décrit l'outil tel qu'il est aujourd'hui.** Pour ça, lire
le [README](README.md), et en particulier
[« L'architecture du tunnel »](README.md#larchitecture-du-tunnel--une-musique-entre-un-amv-sort).

Ce qui est décrit ici n'est pas mort pour autant : le moteur qu'on y voit naître
sert toujours — la lecture des en-têtes, le découpage sur images-clés, l'écriture
du MP4, le coffre et le grenier n'ont pas bougé. Ce sont les gestes à la main qui
ont disparu de l'écran, pas la mécanique qu'ils ont servi à mettre au point.

Les mesures sont datées de leur époque : un chiffre relevé ici l'a été sur le code
de ce moment-là, pas sur celui d'aujourd'hui.
